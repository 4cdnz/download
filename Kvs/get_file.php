...
if ($_SERVER['REQUEST_METHOD'] != 'HEAD' && $range_start == 0)
{
	if (intval($stats_settings['collect_videos_stats_video_files']) == 1 || intval($stats_settings['collect_memberzone_stats_video_files']) == 1)
	{
		...
		file_put_contents("$config[project_path]/admin/data/stats/video_files.dat", "$video_id||" . date("Y-m-d H:i:s") . "||$postfix||$user_id||" . intval($_REQUEST['start_sec']) . "\r\n", LOCK_EX | FILE_APPEND);
	}
}
...
function debug_get_file($message)
{
	...
	if ($config['enable_debug_get_file'] == 'true')
	{
		...
		file_put_contents("$config[project_path]/admin/logs/get_file.txt", "$message\n", LOCK_EX | FILE_APPEND);
	}
}
