...
grid_presets_start($grid_presets, $page_name, 'log_feeds');

$list_feeds = [];
$list_feeds_temp = mr2array(sql("select * from $config[tables_prefix]videos_feeds_import order by title asc"));
foreach ($list_feeds_temp as $feed)
{
	$list_feeds[$feed['feed_id']] = $feed['title'];
}

$list_message_types = array(
		0 => $lang['settings']['feed_log_field_message_type_debug'],
		1 => $lang['settings']['feed_log_field_message_type_info'],
		2 => $lang['settings']['feed_log_field_message_type_error'],
);

$table_fields = array();
$table_fields[] = array('id' => 'record_id',       'title' => $lang['settings']['feed_log_field_id'],           'is_default' => 1, 'type' => 'id');
$table_fields[] = array('id' => 'feed_id',         'title' => $lang['settings']['feed_log_field_feed'],         'is_default' => 1, 'type' => 'choice', 'values' => &$list_feeds, 'link' => 'videos_feeds_import.php?action=change&item_id=%id%', 'link_id' => 'feed_id', 'permission' => 'videos|feeds_import', 'filter_ids' => ['se_feed_id' => 'feed_id']);
$table_fields[] = array('id' => 'message_type',    'title' => $lang['settings']['feed_log_field_message_type'], 'is_default' => 1, 'type' => 'choice', 'values' => $list_message_types, 'ifhighlight' => 'is_error');
$table_fields[] = array('id' => 'message_text',    'title' => $lang['settings']['feed_log_field_message'],      'is_default' => 1, 'type' => 'text', 'ifhighlight' => 'is_error');
$table_fields[] = array('id' => 'message_details', 'title' => $lang['settings']['feed_log_field_details'],      'is_default' => 0, 'type' => 'longtext');
$table_fields[] = array('id' => 'added_date',      'title' => $lang['settings']['feed_log_field_datetime'],     'is_default' => 1, 'type' => 'datetime');
...
{
	$where .= " and feed_id=" . intval($_SESSION['save'][$page_name]['se_feed_id']);
	$table_filtered = 1;
}
...
grid_presets_end($grid_presets, $page_name, 'log_feeds');
...
foreach ($data as $k => $v)
{
	...
	if (!isset($list_feeds[$v['feed_id']]))
	{
		$list_feeds[$v['feed_id']] = str_replace('%1%', $v['feed_id'], $lang['settings']['feed_log_field_feed_deleted']);
	}
}
...
$smarty = new mysmarty();
$smarty->assign('list_feeds', $list_feeds);
