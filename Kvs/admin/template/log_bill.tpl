...
<form action="{{$page_name}}" method="post" class="de de_readonly" name="{{$smarty.now}}" data-editor-name="log_billing_edit">
	<div class="de_main">
		<div class="de_header"><h1><a href="{{$page_name}}">{{$lang.settings.submenu_option_bill_log}}</a> / {{$lang.settings.bill_log_view}}</h1></div>
	...
	<form action="{{$page_name}}" method="get" class="form_dgf" name="{{$smarty.now}}">
		...
		<div class="dgf_advanced">
			...
			<div class="dgf_advanced_filters">
				{{if count($list_satellite_values)>0}}
					<div class="dgf_filter">
						<select name="se_satellite_prefix">
							<option value="">{{$lang.settings.bill_log_field_satellite}}...</option>
							<option value="_" {{if $smarty.session.save.$page_name.se_satellite_prefix=='_'}}selected{{/if}}>{{$lang.settings.bill_log_field_satellite_none}}</option>
							{{foreach from=$list_satellite_values key="value" item="title"}}
								<option value="{{$value}}" {{if $smarty.session.save.$page_name.se_satellite_prefix==$value}}selected{{/if}}>{{$title}}</option>
							{{/foreach}}
						</select>
					</div>
				{{/if}}
				<div class="dgf_filter">
					<select name="se_show_id">
						<option value="">{{$lang.settings.bill_log_field_message_type}}...</option>
						<option value="1" {{if $smarty.session.save.$page_name.se_show_id=='1'}}selected{{/if}}>{{$lang.settings.bill_log_field_message_type_info}}</option>
						<option value="2" {{if $smarty.session.save.$page_name.se_show_id=='2'}}selected{{/if}}>{{$lang.settings.bill_log_field_message_type_error}}</option>
					</select>
				</div>
				<div class="dgf_filter">
					<select name="se_internal_provider_id">
						<option value="">{{$lang.settings.bill_log_field_provider}}...</option>
						{{foreach item="item" key="key" from=$list_providers|smarty:nodefaults}}
							<option value="{{$key}}" {{if $smarty.session.save.$page_name.se_internal_provider_id==$key}}selected{{/if}}>{{$item}}</option>
						{{/foreach}}
					</select>
				</div>
