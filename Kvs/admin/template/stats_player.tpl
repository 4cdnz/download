	...
	<form action="{{$page_name}}" method="get" class="form_dgf" name="{{$smarty.now}}">
		<div class="dgf">
			<div class="dgf_filter">
				<div class="drop">
					<i class="icon icon-action-groupby"></i><span>{{$lang.stats.player_filter_group_by}}</span>
					<ul>
						<li {{if $smarty.session.save.$page_name.se_group_by=='date'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=date">{{$lang.stats.player_filter_group_by_date}}</a></li>
						<li {{if $smarty.session.save.$page_name.se_group_by=='referer'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=referer">{{$lang.stats.player_filter_group_by_referer}}</a></li>
						<li {{if $smarty.session.save.$page_name.se_group_by=='country'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=country">{{$lang.stats.player_filter_group_by_country}}</a></li>
						<li {{if $smarty.session.save.$page_name.se_group_by=='device'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=device">{{$lang.stats.player_filter_group_by_device}}</a></li>
						<li {{if $smarty.session.save.$page_name.se_group_by=='embed_profile'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=embed_profile">{{$lang.stats.player_filter_group_by_embed_profile}}</a></li>
					</ul>
				</div>
			</div>
			<div class="dgf_reset">
			...
			<div class="dgf_advanced_filters">
				<div class="dgf_filter">
					<select name="se_location">
						<option value="">{{$lang.stats.player_filter_location}}...</option>
						<option value="own" {{if $smarty.session.save.$page_name.se_location=='own'}}selected{{/if}}>{{$lang.stats.player_filter_location_own}}</option>
						<option value="embed" {{if $smarty.session.save.$page_name.se_location=='embed'}}selected{{/if}}>{{$lang.stats.player_filter_location_embed}}</option>
					</select>
				</div>
				<div class="dgf_filter">
					<div class="insight">
						<div class="js_params">
							<span class="js_param">url=async/insight.php?type=referers</span>
						</div>
						<input type="text" name="se_referer" value="{{$smarty.session.save.$page_name.se_referer}}" placeholder="{{$lang.stats.common_referer}}..."/>
					</div>
				</div>
				<div class="dgf_filter">
					<div class="insight">
						...
						<input type="text" name="se_country" value="{{$smarty.session.save.$page_name.se_country}}" placeholder="{{$lang.stats.common_country}}..."/>
					</div>
				</div>
				<div class="dgf_filter">
					<select name="se_device">
						<option value="">{{$lang.stats.common_device}}...</option>
						<option value="0" {{if $smarty.session.save.$page_name.se_device=='0'}}selected{{/if}}>{{$lang.stats.common_device_unknown}}</option>
						<option value="1" {{if $smarty.session.save.$page_name.se_device=='1'}}selected{{/if}}>{{$lang.stats.common_device_desktop}}</option>
						<option value="2" {{if $smarty.session.save.$page_name.se_device=='2'}}selected{{/if}}>{{$lang.stats.common_device_phone}}</option>
						<option value="3" {{if $smarty.session.save.$page_name.se_device=='3'}}selected{{/if}}>{{$lang.stats.common_device_tablet}}</option>
					</select>
				</div>
				<div class="dgf_filter">
					<select name="se_embed_profile">
						<option value="">{{$lang.stats.player_field_embed_profile}}...</option>
						{{foreach from=$list_embed_profile_values|smarty:nodefaults item="title" key="key"}}
							<option value="{{$key}}" {{if $smarty.session.save.$page_name.se_embed_profile!='' && $smarty.session.save.$page_name.se_embed_profile==$key}}selected{{/if}}>{{$title}}</option>
						{{/foreach}}
					</select>
				</div>
