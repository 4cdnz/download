...
<form action="{{$page_name}}" method="post" class="de {{if $smarty.post.referer=='&lt;bookmarks&gt;'}}de_readonly{{/if}}" name="{{$smarty.now}}" data-editor-name="referer_edit">
	<div>
	...
	<div class="de_main">
		<div class="de_header"><h1><a href="{{$page_name}}">{{$lang.stats.submenu_option_referers_list}}</a> / {{if $smarty.get.action=='add_new'}}{{$lang.stats.referer_add}}{{else}}{{$lang.stats.referer_edit|replace:"%1%":$smarty.post.title}}{{/if}}</h1></div>
		<table class="de_editor">
			...
			{{if $options.ENABLE_REFERER_FIELD_1==1 || $options.ENABLE_REFERER_FIELD_2==1 || $options.ENABLE_REFERER_FIELD_3==1 || $options.ENABLE_REFERER_FILE_FIELD_1==1 || $options.ENABLE_REFERER_FILE_FIELD_2==1 || $options.ENABLE_REFERER_FILE_FIELD_3==1}}
				<tr>
					<td class="de_separator" colspan="2"><h2>{{$lang.stats.referer_divider_general}}</h2></td>
				</tr>
			{{/if}}
			<tr>
			...
			<tr>
				...
				<td class="de_control"><textarea name="description" class="{{if $smarty.session.userdata.is_wysiwyg_enabled_other=='1'}}tinymce{{/if}}" cols="40" rows="3">{{$smarty.post.description}}</textarea></td>
			</tr>
			<tr>
				<td class="de_label">{{$lang.stats.referer_field_url}}</td>
				<td class="de_control">
					<input type="text" name="url" maxlength="255" value="{{$smarty.post.url}}"/>
					<span class="de_hint">{{$lang.stats.referer_field_url_hint}}</span>
				</td>
			</tr>
			<tr>
				...
				<td class="de_control">
					<div class="insight">
						...
						<input type="text" name="category" maxlength="255" value="{{$smarty.post.category.title}}"/>
					</div>
				</td>
			</tr>
			<tr>
				...
				<td class="de_control">
					<input type="text" name="referer" maxlength="255" value="{{$smarty.post.referer}}"/>
					<span class="de_hint">{{$lang.stats.referer_field_referer_hint}}</span>
				</td>
			</tr>
			{{if $options.ENABLE_REFERER_FIELD_1==1 || $options.ENABLE_REFERER_FIELD_2==1 || $options.ENABLE_REFERER_FIELD_3==1 || $options.ENABLE_REFERER_FILE_FIELD_1==1 || $options.ENABLE_REFERER_FILE_FIELD_2==1 || $options.ENABLE_REFERER_FILE_FIELD_3==1}}
				<tr>
					<td class="de_separator" colspan="2"><h2>{{$lang.stats.referer_divider_customization}}</h2></td>
				</tr>
				{{if $options.ENABLE_REFERER_FIELD_1==1}}
					<tr>
						<td class="de_label">{{$options.REFERER_FIELD_1_NAME}}</td>
						<td class="de_control"><textarea name="custom1" class="{{if $smarty.session.userdata.is_wysiwyg_enabled_other=='1'}}tinymce{{/if}}" cols="40" rows="4">{{$smarty.post.custom1}}</textarea></td>
					</tr>
				{{/if}}
				{{if $options.ENABLE_REFERER_FIELD_2==1}}
					<tr>
						<td class="de_label">{{$options.REFERER_FIELD_2_NAME}}</td>
						<td class="de_control"><textarea name="custom2" class="{{if $smarty.session.userdata.is_wysiwyg_enabled_other=='1'}}tinymce{{/if}}" cols="40" rows="4">{{$smarty.post.custom2}}</textarea></td>
					</tr>
				{{/if}}
				{{if $options.ENABLE_REFERER_FIELD_3==1}}
					<tr>
						<td class="de_label">{{$options.REFERER_FIELD_3_NAME}}</td>
						<td class="de_control"><textarea name="custom3" class="{{if $smarty.session.userdata.is_wysiwyg_enabled_other=='1'}}tinymce{{/if}}" cols="40" rows="4">{{$smarty.post.custom3}}</textarea></td>
					</tr>
				{{/if}}
			...
			<div class="dgf_advanced_filters">
				<div class="dgf_filter">
					<div class="insight">
						...
						<input type="text" name="se_category" value="{{$smarty.session.save.$page_name.se_category}}" placeholder="{{$lang.stats.referer_field_category}}"/>
					</div>
				</div>
			</div>
				...
				<tbody>
					...
					{{foreach name="data" item="item" from=$data|smarty:nodefaults}}
						<tr class="dg_data{{if $smarty.foreach.data.iteration % 2==0}} dg_even{{/if}}">
							<td class="dg_selector"><input type="checkbox" name="row_select[]" value="{{$item.$table_key_name}}" {{if $item.referer=='&lt;bookmarks&gt;'}}disabled{{/if}}/></td>
