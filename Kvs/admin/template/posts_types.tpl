...
<form action="{{$page_name}}" method="post" class="de {{if $can_edit_all==0}}de_readonly{{/if}}" name="{{$smarty.now}}" data-editor-name="post_type_edit">
	<div class="de_main">
		<div class="de_header"><h1><a href="{{$page_name}}">{{$lang.posts.submenu_option_post_types_list}}</a> / {{if $smarty.get.action=='add_new'}}{{$lang.posts.post_type_add}}{{else}}{{$lang.posts.post_type_edit|replace:"%1%":$smarty.post.title}}{{/if}}</h1></div>
		<table class="de_editor">
			...
			<tr>
				...
				<td class="de_control">
					...
					<span class="de_hint">{{$lang.posts.post_type_field_external_id_hint}}</span>
				</td>
			</tr>
			<tr>
				<td class="de_label de_required">{{$lang.posts.post_type_field_url_pattern}}</td>
				<td class="de_control">
					<input type="text" name="url_pattern" maxlength="255" value="{{$smarty.post.url_pattern}}"/>
					<span class="de_hint">{{$lang.posts.post_type_field_url_pattern_hint}}</span>
				</td>
			</tr>
			<tr>
			...
			<div class="dgf_advanced_filters">
				<div class="dgf_filter">
					<select name="se_field">
						...
						<option value="empty/description" {{if $smarty.session.save.$page_name.se_field=="empty/description"}}selected{{/if}}>{{$lang.common.dg_filter_option_empty_field|replace:"%1%":$lang.posts.post_type_field_description}}</option>
						<option value="filled/description" {{if $smarty.session.save.$page_name.se_field=="filled/description"}}selected{{/if}}>{{$lang.common.dg_filter_option_non_empty_field|replace:"%1%":$lang.posts.post_type_field_description}}</option>
					</select>
				</div>
			</div>
			...
			<div class="dgb_actions">
				<select name="batch_action">
					...
					{{if $can_delete==1}}
						...
						{{if in_array('posts|delete',$smarty.session.permissions)}}
							<option value="delete_with_content">{{$lang.posts.post_type_batch_delete_with_content|replace:"%1%":'${count}'}}</option>
						{{/if}}
					{{/if}}
				</select>
