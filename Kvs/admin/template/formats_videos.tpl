...
<form action="{{$page_name}}" method="post" class="de" name="{{$smarty.now}}" data-editor-name="format_video_group">
	<div class="de_main">
		<div class="de_header"><h1><a href="{{$page_name}}">{{$lang.settings.submenu_option_formats_videos_list}}</a> / {{if $smarty.get.action=='add_new_group'}}{{$lang.settings.format_video_group_add}}{{else}}{{$lang.settings.format_video_group_edit|replace:"%1%":$smarty.post.title}}{{/if}}</h1></div>
		<table class="de_editor">
			...
			<tr>
				<td class="de_label de_required">{{$lang.settings.format_video_group_field_title}}</td>
				<td class="de_control">
					<input type="text" name="title" maxlength="100" value="{{$smarty.post.title}}"/>
				</td>
			</tr>
			{{if $smarty.get.action!='add_new_group'}}
				{{if count($smarty.post.required_formats)>0}}
					<tr>
						<td class="de_label">{{$lang.settings.format_video_group_field_default}}</td>
						<td class="de_control">
							<span class="de_lv_pair"><input type="checkbox" name="is_default" value="1" {{if $smarty.post.is_default==1}}checked disabled{{/if}}/><label>{{$lang.settings.format_video_group_field_default_yes}}</label></span>
						</td>
					</tr>
					<tr>
						<td class="de_label">{{$lang.settings.format_video_group_field_premium}}</td>
						<td class="de_control">
							<span class="de_lv_pair"><input type="checkbox" name="is_premium" value="1" {{if $smarty.post.is_premium==1}}checked{{/if}}/><label>{{$lang.settings.format_video_group_field_premium_yes}}</label></span>
						</td>
					</tr>
				{{/if}}
				<tr>
					<td class="de_label">{{$lang.settings.format_video_group_set_duration_from}}</td>
					<td class="de_control">
						<select name="set_duration_from">
							<option value="">{{$lang.settings.format_video_group_set_duration_from_auto}}</option>
							{{foreach from=$smarty.post.required_formats item="format"}}
								<option value="{{$format.postfix}}" {{if $format.postfix==$smarty.post.set_duration_from}}selected{{/if}}>{{$lang.settings.format_video_group_set_duration_from_format|replace:"%1%":$format.title}}</option>
							{{/foreach}}
						</select>
						<span class="de_hint">{{$lang.settings.format_video_group_set_duration_from_hint}}</span>
					</td>
				</tr>
			{{/if}}
		</table>
...
<form action="{{$page_name}}" method="post" class="de {{if $smarty.post.status_id==3 || $smarty.post.status_id==4}}de_readonly{{/if}}" name="{{$smarty.now}}" data-editor-name="format_video">
	<div class="de_main">
		<div class="de_header"><h1><a href="{{$page_name}}">{{$lang.settings.submenu_option_formats_videos_list}}</a> / {{if $smarty.get.action=='add_new'}}{{$lang.settings.format_video_add}}{{else}}{{$lang.settings.format_video_edit|replace:"%1%":$smarty.post.title}}{{/if}}</h1></div>
		<table class="de_editor">
			<tr class="err_list {{if !is_array($smarty.post.errors)}}hidden{{/if}}">
				<td colspan="2">
					<div class="err_header">{{$lang.validation.common_header}}</div>
			...
			<tr>
				...
				<td class="de_control">
					<input type="text" name="postfix" maxlength="32" size="32" value="{{$smarty.post.postfix}}" {{if $smarty.get.action!='add_new'}}disabled{{/if}}/>
					<span class="de_hint">{{$lang.settings.format_video_field_postfix_hint|replace:"%1%":$allowed_formats}}</span>
				</td>
			</tr>
			<tr>
				<td class="de_label">{{$lang.settings.format_video_field_format_video_group}}</td>
				<td class="de_control">
					<select name="format_video_group_id" {{if $smarty.get.action!='add_new'}}disabled{{/if}}>
						{{foreach item="group" from=$list_format_groups|smarty:nodefaults}}
							<option value="{{$group.format_video_group_id}}" {{if $smarty.post.format_video_group_id==$group.format_video_group_id}}selected{{/if}}>{{$lang.settings.format_video_field_format_video_group_value|replace:"%1%":$group.title|replace:"%2%":$group.videos_count}}</option>
						{{/foreach}}
					</select>
					<span class="de_hint">{{$lang.settings.format_video_field_format_video_group_hint}}</span>
				</td>
			</tr>
			<tr>
				...
				<td class="de_control de_vis_sw_select">
					<select name="status_id">
						{{if $smarty.get.action!='add_new'}}
							<option value="0" {{if $smarty.post.status_id==0}}selected{{/if}}>{{$lang.settings.format_video_field_status_disabled}}</option>
						{{/if}}
						<option value="1" {{if $smarty.post.status_id==1}}selected{{/if}}>{{$lang.settings.format_video_field_status_active_required}}</option>
						<option value="2" {{if $smarty.post.status_id==2}}selected{{/if}}>{{$lang.settings.format_video_field_status_active_optional}}</option>
					</select>
					<span class="de_lv_pair status_id_2"><input type="checkbox" name="is_conditional" value="1" {{if $smarty.post.is_conditional==1}}checked{{/if}}/><label>{{$lang.settings.format_video_field_status_active_optional_conditional}}</label></span>
					{{if $smarty.get.action!='add_new'}}
					...
					<span class="de_hint status_id_1">{{$lang.settings.format_video_warning|replace:"%1%":$lang.settings.format_video_field_status_active_optional}}</span>
			...
			<tr>
				...
				<td class="de_control">
					<span class="de_lv_pair"><input type="checkbox" name="is_use_as_source" value="1" {{if $smarty.post.is_use_as_source==1}}checked{{/if}}/><label>{{$lang.settings.format_video_field_use_as_source_yes}}</label></span>
					<span class="de_hint">{{$lang.settings.format_video_field_use_as_source_hint}}</span>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="de_control">
					<select name="access_level_id">
						<option value="0" {{if $smarty.post.access_level_id==0}}selected{{/if}}>{{$lang.settings.format_video_field_access_level_any}}</option>
						<option value="1" {{if $smarty.post.access_level_id==1}}selected{{/if}}>{{$lang.settings.format_video_field_access_level_member}}</option>
						<option value="2" {{if $smarty.post.access_level_id==2}}selected{{/if}}>{{$lang.settings.format_video_field_access_level_premium}}</option>
					</select>
					<span class="de_hint">{{$lang.settings.format_video_field_access_level_hint}}</span>
				</td>
			</tr>
			<tr>
			...
			<tr class="is_download_enabled_on">
				...
				<td class="de_control">
					<input type="text" name="download_order" maxlength="10" size="10" value="{{$smarty.post.download_order}}"/>
					<span class="de_hint">{{$lang.settings.format_video_field_enable_download_order_hint}}</span>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="de_control">
					<table class="control_group">
						<tr>
							<td>
								<span class="de_lv_pair de_vis_sw_checkbox"><input type="checkbox" name="limit_speed_guests_option_override" value="1" {{if $smarty.post.limit_speed_guests_option!=$smarty.post.limit_speed_option || $smarty.post.limit_speed_guests_value!=$smarty.post.limit_speed_value}}checked{{/if}}/><label>{{$lang.settings.format_video_field_limit_speed_override}}</label></span>
							</td>
						</tr>
						<tr class="limit_speed_guests_option_override_on">
							<td class="de_vis_sw_select">
								<select name="limit_speed_guests_option">
									<option value="0" {{if $smarty.post.limit_speed_guests_option==0}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_unlimited}}</option>
									<option value="1" {{if $smarty.post.limit_speed_guests_option==1}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_fixed}}</option>
									<option value="2" {{if $smarty.post.limit_speed_guests_option==2}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_dynamic}}</option>
								</select>
								<span>
									<input type="text" name="limit_speed_guests_value" maxlength="10" size="5" class="limit_speed_guests_option_1 limit_speed_guests_option_2" value="{{$smarty.post.limit_speed_guests_value}}"/>
			...
			<tr>
				...
				<td class="de_control">
					<table class="control_group">
						<tr>
							<td>
								<span class="de_lv_pair de_vis_sw_checkbox"><input type="checkbox" name="limit_speed_standard_option_override" value="1" {{if $smarty.post.limit_speed_standard_option!=$smarty.post.limit_speed_option || $smarty.post.limit_speed_standard_value!=$smarty.post.limit_speed_value}}checked{{/if}}/><label>{{$lang.settings.format_video_field_limit_speed_override}}</label></span>
							</td>
						</tr>
						<tr class="limit_speed_standard_option_override_on">
							<td class="de_vis_sw_select">
								<select name="limit_speed_standard_option">
									<option value="0" {{if $smarty.post.limit_speed_standard_option==0}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_unlimited}}</option>
									<option value="1" {{if $smarty.post.limit_speed_standard_option==1}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_fixed}}</option>
									<option value="2" {{if $smarty.post.limit_speed_standard_option==2}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_dynamic}}</option>
								</select>
								<span>
									<input type="text" name="limit_speed_standard_value" maxlength="10" size="5" class="limit_speed_standard_option_1 limit_speed_standard_option_2" value="{{$smarty.post.limit_speed_standard_value}}"/>
			...
			<tr>
				...
				<td class="de_control">
					<table class="control_group">
						<tr>
							<td>
								<span class="de_lv_pair de_vis_sw_checkbox"><input type="checkbox" name="limit_speed_premium_option_override" value="1" {{if $smarty.post.limit_speed_premium_option!=$smarty.post.limit_speed_option || $smarty.post.limit_speed_premium_value!=$smarty.post.limit_speed_value}}checked{{/if}}/><label>{{$lang.settings.format_video_field_limit_speed_override}}</label></span>
							</td>
						</tr>
						<tr class="limit_speed_premium_option_override_on">
							<td class="de_vis_sw_select">
								<select name="limit_speed_premium_option">
									<option value="0" {{if $smarty.post.limit_speed_premium_option==0}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_unlimited}}</option>
									<option value="1" {{if $smarty.post.limit_speed_premium_option==1}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_fixed}}</option>
									<option value="2" {{if $smarty.post.limit_speed_premium_option==2}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_dynamic}}</option>
								</select>
								<span>
									<input type="text" name="limit_speed_premium_value" maxlength="10" size="5" class="limit_speed_premium_option_1 limit_speed_premium_option_2" value="{{$smarty.post.limit_speed_premium_value}}"/>
			...
			<tr>
				...
				<td class="de_control">
					<table class="control_group">
						<tr>
							<td>
								<span class="de_lv_pair de_vis_sw_checkbox"><input type="checkbox" name="limit_speed_embed_option_override" value="1" {{if $smarty.post.limit_speed_embed_option!=$smarty.post.limit_speed_option || $smarty.post.limit_speed_embed_value!=$smarty.post.limit_speed_value}}checked{{/if}}/><label>{{$lang.settings.format_video_field_limit_speed_override}}</label></span>
							</td>
						</tr>
						<tr class="limit_speed_embed_option_override_on">
							<td class="de_vis_sw_select">
								<select name="limit_speed_embed_option">
									<option value="0" {{if $smarty.post.limit_speed_embed_option==0}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_unlimited}}</option>
									<option value="1" {{if $smarty.post.limit_speed_embed_option==1}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_fixed}}</option>
									<option value="2" {{if $smarty.post.limit_speed_embed_option==2}}selected{{/if}}>{{$lang.settings.format_video_field_limit_speed_option_dynamic}}</option>
								</select>
								<span>
									<input type="text" name="limit_speed_embed_value" maxlength="10" size="5" class="limit_speed_embed_option_1 limit_speed_embed_option_2" value="{{$smarty.post.limit_speed_embed_value}}"/>
	...
	<form action="{{$page_name}}" method="post" class="form_dg" name="{{$smarty.now}}">
		<div class="dg">
					...
					{{foreach item="group" from=$list_format_groups|smarty:nodefaults}}
						<tr class="dg_group_header">
							<td colspan="{{$table_columns_visible-1}}">
								<a href="{{$page_name}}?action=change_group&amp;item_id={{$group.format_video_group_id}}">{{$group.title}} ({{$group.videos_count}}){{if $group.is_default==1}} - {{$lang.settings.format_video_group_field_default}}{{/if}}</a>
							</td>
							<td class="nowrap">
								<a {{if $group.is_editing_forbidden!=1}}href="{{$page_name}}?action=change_group&amp;item_id={{$group.format_video_group_id}}"{{/if}} class="edit {{if $group.is_editing_forbidden==1}}disabled{{/if}}" title="{{$lang.common.dg_actions_edit}}"><i class="icon icon-action-edit"></i></a>
								<a class="additional" title="{{$lang.common.dg_actions_additional}}">
									<i class="icon icon-action-settings"></i>
									<span class="js_params">
										<span class="js_param">id={{$group.format_video_group_id}}</span>
										<span class="js_param">name={{$group.title}}</span>
										<span class="js_param">delete_hide=true</span>
										<span class="js_param">delete_timelines_hide=true</span>
										{{if count($data[$group.format_video_group_id])>0}}
											<span class="js_param">delete_group_disable=true</span>
										{{/if}}
									</span>
								</a>
							</td>
						...
						{{if count($data[$group.format_video_group_id])>0}}
							{{foreach name="data" item="item" from=$data[$group.format_video_group_id]|smarty:nodefaults}}
			...
			<ul class="dg_additional_menu_template">
				...
				<li class="js_params">
					<span class="js_param">href=?batch_action=delete_group&amp;row_select[]=${id}</span>
					<span class="js_param">title={{$lang.common.dg_actions_delete}}</span>
					...
					<span class="js_param">disable=${delete_group_disable}</span>
					<span class="js_param">icon=action-delete</span>
					<span class="js_param">destructive=true</span>
				</li>
				<li class="js_params">
