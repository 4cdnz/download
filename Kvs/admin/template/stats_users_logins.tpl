	...
	<form action="{{$page_name}}" method="get" class="form_dgf" name="{{$smarty.now}}">
		<div class="dgf">
			<div class="dgf_filter">
				<div class="drop">
					<i class="icon icon-action-groupby"></i><span>{{$lang.stats.users_logins_filter_group_by}}</span>
					<ul>
						<li {{if $smarty.session.save.$page_name.se_group_by=='user'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=user">{{$lang.stats.users_logins_filter_group_by_user}}</a></li>
						<li {{if $smarty.session.save.$page_name.se_group_by=='ip'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=ip">{{$lang.stats.users_logins_filter_group_by_ip}}</a></li>
						<li {{if $smarty.session.save.$page_name.se_group_by=='log'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=log">{{$lang.stats.users_logins_filter_group_by_full}}</a></li>
					</ul>
				</div>
			</div>
			<div class="dgf_reset">
			...
			<div class="dgf_advanced_filters">
				<div class="dgf_filter">
					<div class="insight">
						...
						<input type="text" name="se_user" value="{{$smarty.session.save.$page_name.se_user}}" placeholder="{{$lang.stats.users_logins_field_user}}..."/>
					</div>
				</div>
				<div class="dgf_filter">
					<select name="se_status_id">
						<option value="">{{$lang.stats.users_logins_field_status}}...</option>
						<option value="0" {{if $smarty.session.save.$page_name.se_status_id=='0'}}selected{{/if}}>{{$lang.stats.users_logins_field_status_disabled}}</option>
						<option value="2" {{if $smarty.session.save.$page_name.se_status_id=='2'}}selected{{/if}}>{{$lang.stats.users_logins_field_status_active}}</option>
						<option value="3" {{if $smarty.session.save.$page_name.se_status_id=='3'}}selected{{/if}}>{{$lang.stats.users_logins_field_status_premium}}</option>
						<option value="6" {{if $smarty.session.save.$page_name.se_status_id=='6'}}selected{{/if}}>{{$lang.stats.users_logins_field_status_webmaster}}</option>
					</select>
				</div>
				<div class="dgf_filter">
					<input type="text" name="se_ip" value="{{$smarty.session.save.$page_name.se_ip}}" placeholder="{{$lang.stats.users_logins_field_ip}}..."/>
				</div>
				<div class="dgf_filter">
					<input type="text" name="se_reseller_code" value="{{$smarty.session.save.$page_name.se_reseller_code}}" placeholder="{{$lang.stats.users_logins_field_reseller_code}}..."/>
				</div>
