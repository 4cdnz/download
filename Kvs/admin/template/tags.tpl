...
<form action="{{$page_name}}" method="post" class="de {{if $can_edit_all==0}}de_readonly{{/if}}" name="{{$smarty.now}}" data-editor-name="tag_edit">
	<div>
	...
	<div class="de_main">
		<div class="de_header"><h1><a href="{{$page_name}}">{{$lang.categorization.submenu_option_tags_list}}</a> / {{if $smarty.get.action=='add_new'}}{{$lang.categorization.tag_add}}{{else}}{{$lang.categorization.tag_edit|replace:"%1%":$smarty.post.tag}}{{/if}}</h1></div>
		<table class="de_editor">
			...
			{{if $smarty.get.action!='add_new' && ($options.ENABLE_TAG_FIELD_1==1 || $options.ENABLE_TAG_FIELD_2==1 || $options.ENABLE_TAG_FIELD_3==1 || $options.ENABLE_TAG_FIELD_4==1 || $options.ENABLE_TAG_FIELD_5==1)}}
				<tr>
					<td class="de_separator" colspan="3"><h2>{{$lang.categorization.tag_divider_general}}</h2></td>
				</tr>
			{{/if}}
			<tr>
				{{if $smarty.get.action=='add_new'}}
					<td class="de_label de_required">{{$lang.categorization.tag_field_tags}}</td>
					<td class="de_control">
						<textarea name="tag" cols="40" rows="8"></textarea>
						<span class="de_hint">{{$lang.categorization.tag_field_tags_hint}}</span>
					</td>
				{{else}}
					<td class="de_label de_required">{{$lang.categorization.tag_field_tag}}</td>
					<td class="de_control">
						<input type="text" name="tag" maxlength="150"  value="{{$smarty.post.tag}}"/>
			...
			{{if $smarty.get.action=='change'}}
				<tr>
					...
					<td class="de_control">
						<input type="text" name="tag_dir" maxlength="255" value="{{$smarty.post.tag_dir}}"/>
						<span class="de_hint">{{$lang.categorization.tag_field_directory_hint|replace:"%1%":$lang.categorization.tag_field_tag}}</span>
					</td>
				</tr>
				<tr>
					...
					<td class="de_control">
						...
						<span class="de_hint">{{$lang.categorization.tag_field_synonyms_hint}}</span>
					</td>
				</tr>
				<tr>
					<td class="de_label">{{$lang.categorization.tag_field_status}}</td>
					<td class="de_control">
						<span class="de_lv_pair"><input type="checkbox" name="status_id" value="1" {{if $smarty.post.status_id=='1'}}checked{{/if}}/><label>{{$lang.categorization.tag_field_status_active}}</label></span>
						<span class="de_hint">{{$lang.categorization.tag_field_status_hint}}</span>
					</td>
				</tr>
				{{if $options.ENABLE_TAG_FIELD_1==1 || $options.ENABLE_TAG_FIELD_2==1 || $options.ENABLE_TAG_FIELD_3==1 || $options.ENABLE_TAG_FIELD_4==1 || $options.ENABLE_TAG_FIELD_5==1}}
					<tr>
						<td class="de_separator" colspan="3"><h2>{{$lang.categorization.tag_divider_customization}}</h2></td>
					</tr>
			...
			<div class="dgf_advanced_filters">
				<div class="dgf_filter">
					<select name="se_status_id">
						<option value="">{{$lang.categorization.tag_field_status}}...</option>
						<option value="1" {{if $smarty.session.save.$page_name.se_status_id=='1'}}selected{{/if}}>{{$lang.categorization.tag_field_status_active}}</option>
						<option value="0" {{if $smarty.session.save.$page_name.se_status_id=='0'}}selected{{/if}}>{{$lang.categorization.tag_field_status_disabled}}</option>
					</select>
				</div>
				<div class="dgf_filter">
					<select name="se_field">
						<option value="">{{$lang.common.dg_filter_field}}...</option>
						<option value="empty/synonyms" {{if $smarty.session.save.$page_name.se_field=="empty/synonyms"}}selected{{/if}}>{{$lang.common.dg_filter_option_empty_field|replace:"%1%":$lang.categorization.tag_field_synonyms}}</option>
						{{section name="data" start="1" loop="6"}}
							...
							{{/if}}
						{{/section}}
						<option value="filled/synonyms" {{if $smarty.session.save.$page_name.se_field=="filled/synonyms"}}selected{{/if}}>{{$lang.common.dg_filter_option_non_empty_field|replace:"%1%":$lang.categorization.tag_field_synonyms}}</option>
						{{section name="data" start="1" loop="6"}}
							{{assign var="custom_field_id" value="custom`$smarty.section.data.index`"}}
							{{assign var="custom_field_name_id" value="TAG_FIELD_`$smarty.section.data.index`_NAME"}}
							{{assign var="custom_field_enable_id" value="ENABLE_TAG_FIELD_`$smarty.section.data.index`"}}
	...
	<form action="{{$page_name}}" method="post" class="form_dg" name="{{$smarty.now}}">
		...
		<div class="dgb">
					...
					{{if $can_edit==1}}
						...
						{{foreach from=$table_fields|smarty:nodefaults item="field"}}
							{{if $field.type=='rename' && $field.is_enabled==1}}
								<option value="rename">{{$lang.categorization.tag_batch_action_rename|replace:"%1%":'${all}'}}</option>
							{{/if}}
						{{/foreach}}
					{{/if}}
				</select>
			...
			<ul class="dgb_actions_configuration">
				...
				<li class="js_params">
					<span class="js_param">value=rename</span>
					<span class="js_param">requires_selection=false</span>
					<span class="js_param">confirm={{$lang.categorization.tag_batch_action_rename_confirm|replace:"%1%":'${all}'}}</span>
				</li>
			</ul>
		</div>
	</form>
