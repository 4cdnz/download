	...
	<form action="{{$page_name}}" method="get" class="form_dgf" name="{{$smarty.now}}">
		<div class="dgf">
			<div class="dgf_filter">
				<div class="drop">
					<i class="icon icon-action-groupby"></i><span>{{$lang.stats.users_donations_filter_group_by}}</span>
					<ul>
						<li {{if $smarty.session.save.$page_name.se_group_by=='donator'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=donator">{{$lang.stats.users_donations_filter_group_by_donator}}</a></li>
						<li {{if $smarty.session.save.$page_name.se_group_by=='user'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=user">{{$lang.stats.users_donations_filter_group_by_user}}</a></li>
						<li {{if $smarty.session.save.$page_name.se_group_by=='log'}}class="selected"{{/if}}><a href="{{$page_name}}?se_group_by=log">{{$lang.stats.users_donations_filter_group_by_full}}</a></li>
					</ul>
				</div>
			</div>
			<div class="dgf_reset">
					...
					<div class="insight">
						<div class="js_params">
							...
							{{if in_array('users|view',$smarty.session.permissions)}}
								...
								<span class="js_param">view_id={{$smarty.session.save.$page_name.se_donator_id}}</span>
							{{/if}}
						</div>
						<input type="text" name="se_donator" value="{{$smarty.session.save.$page_name.se_donator}}" placeholder="{{$lang.stats.users_donations_field_donator}}..."/>
					</div>
				</div>
				<div class="dgf_filter">
					<div class="insight">
						...
						<input type="text" name="se_user" value="{{$smarty.session.save.$page_name.se_user}}" placeholder="{{$lang.stats.users_donations_field_user}}..."/>
					</div>
				</div>
				<div class="dgf_filter">
					<select name="se_status_id">
						<option value="">{{$lang.stats.users_donations_field_status}}...</option>
						<option value="0" {{if $smarty.session.save.$page_name.se_status_id=='0'}}selected{{/if}}>{{$lang.stats.users_donations_field_status_disabled}}</option>
						<option value="2" {{if $smarty.session.save.$page_name.se_status_id=='2'}}selected{{/if}}>{{$lang.stats.users_donations_field_status_active}}</option>
						<option value="3" {{if $smarty.session.save.$page_name.se_status_id=='3'}}selected{{/if}}>{{$lang.stats.users_donations_field_status_premium}}</option>
						<option value="6" {{if $smarty.session.save.$page_name.se_status_id=='6'}}selected{{/if}}>{{$lang.stats.users_donations_field_status_webmaster}}</option>
					</select>
				</div>
				<div class="dgf_filter">
					<input type="text" name="se_reseller_code" value="{{$smarty.session.save.$page_name.se_reseller_code}}" placeholder="{{$lang.stats.users_donations_field_reseller_code}}..."/>
				</div>
