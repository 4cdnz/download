...
<form action="{{$page_name}}" method="post" class="de {{if $can_edit_all==0}}de_readonly{{/if}}" name="{{$smarty.now}}" data-editor-name="theme_text_edit">
	<div class="de_main">
		<div class="de_header"><h1><a href="{{$page_name}}">{{$lang.website_ui.submenu_option_text_items}}</a> / {{if $smarty.get.action=='add_new'}}{{$lang.website_ui.text_item_add}}{{else}}{{$lang.website_ui.text_item_edit|replace:"%1%":$smarty.post.external_id}}{{/if}}</h1></div>
			...
			{{if $smarty.get.action=='add_new'}}
				<tr>
					...
					<td class="de_control">
						...
						<span class="de_hint">{{$lang.website_ui.text_item_field_id_hint}}</span>
					</td>
				</tr>
			{{else}}
				<tr>
					...
					<td class="de_control">
						<span>{{$smarty.post.external_id}}</span>
					</td>
				</tr>
			{{/if}}
			<tr>
				...
				<td class="de_control">
					{{if $smarty.get.action=='add_new'}}
						<input type="text" data-autopopulate-from="external_id" data-autopopulate-pattern="{{$smarty.ldelim}}$lang.${value}{{$smarty.rdelim}}" readonly/>
					{{else}}
						<input type="text" value="{{$smarty.ldelim}}$lang.{{$smarty.post.external_id}}{{$smarty.rdelim}}" readonly/>
					{{/if}}
					<span class="de_hint">{{$lang.website_ui.text_item_field_insert_code_hint}}</span>
				</td>
			</tr>
			<tr>
				<td class="de_label de_required">{{$lang.website_ui.text_item_field_text_default}} {{if $default_lang_title}}({{$default_lang_title}}){{/if}}</td>
				<td class="de_control">
					<input type="text" name="text_default" value="{{$smarty.post.text_default}}"/>
					<span class="de_hint">
				...
				{{if $language.code!=$default_lang}}
					<tr>
						...
						<td class="de_control">
							...
							<input type="text" name="text_{{$language_code}}" value="{{$smarty.post.$language_key}}"/>
							<span class="de_hint">
								{{if $smarty.post.is_url==1}}
			...
			<div class="dgf_advanced_filters">
				<div class="dgf_filter">
					<select name="se_prefix">
						<option value="">{{$lang.website_ui.text_item_filter_prefix}}...</option>
						<option value="urls" {{if $smarty.session.save.$page_name.se_prefix=='urls'}}selected{{/if}}>{{$lang.website_ui.text_item_filter_prefix_urls}}</option>
						<option value="html" {{if $smarty.session.save.$page_name.se_prefix=='html'}}selected{{/if}}>{{$lang.website_ui.text_item_filter_prefix_html}}</option>
					</select>
				</div>
				<div class="dgf_filter">
					<select name="se_page">
						<option value="">{{$lang.website_ui.text_item_filter_page}}...</option>
						{{foreach from=$pages|smarty:nodefaults item="page"}}
							<option value="{{$page.external_id}}" {{if $smarty.session.save.$page_name.se_page==$page.external_id}}selected{{/if}}>{{$page.title}}</option>
						{{/foreach}}
					</select>
				</div>
				<div class="dgf_filter">
					<select name="se_missing_translation">
						<option value="">{{$lang.website_ui.text_item_filter_missing_translation}}...</option>
						{{foreach from=$languages|smarty:nodefaults item="language"}}
							{{if $language.code!=$default_lang}}
								<option value="{{$language.code}}" {{if $smarty.session.save.$page_name.se_missing_translation==$language.code}}selected{{/if}}>{{$language.title}}</option>
							{{/if}}
						{{/foreach}}
					</select>
				</div>
	...
	<form action="{{$page_name}}" method="post" class="form_dg" name="{{$smarty.now}}">
		<div class="dg">
			...
			<ul class="dg_additional_menu_template">
				...
				{{/if}}
				<li class="js_params">
					<span class="js_param">href=templates_search.php?no_filter=true&amp;se_text=%24lang.${id}&amp;se_text_templates=1</span>
					<span class="js_param">title={{$lang.website_ui.text_item_action_find_usages}}</span>
					<span class="js_param">plain_link=true</span>
					<span class="js_param">icon=action-search</span>
				</li>
			</ul>
		</div>
		<div class="dgb">
