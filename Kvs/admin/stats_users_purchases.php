...
grid_presets_start($grid_presets, $page_name, 'stats_users_purchases');
...
{
	$table_fields[] = array('id' => 'user',                    'title' => $lang['stats']['users_purchases_field_user'],                    'is_default' => 1, 'type' => 'user', 'filter_ids' => ['se_user' => 'user']);
	$table_fields[] = array('id' => 'reseller_code',           'title' => $lang['stats']['users_purchases_field_reseller_code'],           'is_default' => 0, 'type' => 'text', 'filter_ids' => ['se_reseller_code' => 'reseller_code']);
	...
	$table_fields[] = array('id' => 'messages_purchased',      'title' => $lang['stats']['users_purchases_field_messages_purchased'],      'is_default' => 1, 'type' => 'number', 'ifdisable_zero' => 1);
...
} else
{
	$table_fields[] = array('id' => 'user',            'title' => $lang['stats']['users_purchases_field_user'],            'is_default' => 1, 'type' => 'user', 'filter_ids' => ['se_user' => 'user']);
	$table_fields[] = array('id' => 'reseller_code',   'title' => $lang['stats']['users_purchases_field_reseller_code'],   'is_default' => 0, 'type' => 'text', 'filter_ids' => ['se_reseller_code' => 'reseller_code']);
	$table_fields[] = array('id' => 'object',          'title' => $lang['stats']['users_purchases_field_object'],          'is_default' => 1, 'type' => 'object', 'filter_ids' => ['se_object_type_id' => 'object_type_id', 'se_object_id' => 'object_id']);
	$table_fields[] = array('id' => 'tokens_spent',    'title' => $lang['stats']['users_purchases_field_tokens_spent'],    'is_default' => 1, 'type' => 'number', 'ifdisable_zero' => 1);
	$table_fields[] = array('id' => 'tokens_revenue',  'title' => $lang['stats']['users_purchases_field_tokens_revenue'],  'is_default' => 0, 'type' => 'number', 'ifdisable_zero' => 1);
	...
	$table_fields[] = array('id' => 'is_subscription', 'title' => $lang['stats']['users_purchases_field_is_subscription'], 'is_default' => 1, 'type' => 'bool');
	$table_fields[] = array('id' => 'is_recurring',    'title' => $lang['stats']['users_purchases_field_is_recurring'],    'is_default' => 1, 'type' => 'bool');
}
...
{
	$table_selector = "$table_name.*, $table_name.tokens as tokens_spent, $table_name.user_id, $config[tables_prefix]users.username as user, $config[tables_prefix]users.status_id as user_status_id, $config[tables_prefix]users.reseller_code, case when $table_name.video_id>0 then $table_name.video_id when $table_name.album_id>0 then $table_name.album_id when $table_name.dvd_id>0 then $table_name.dvd_id when $table_name.profile_id>0 then $table_name.profile_id end as object_id, case when $table_name.video_id>0 then 1 when $table_name.album_id>0 then 2 when $table_name.dvd_id>0 then 5 when $table_name.profile_id>0 then 20 when $table_name.messages>0 then 21 end as object_type_id, case when $table_name.messages>0 then $table_name.messages else 0 end as object_amount, case when $table_name.video_id>0 then coalesce($config[tables_prefix]videos.title, $config[tables_prefix]videos.video_id) when $table_name.album_id>0 then coalesce($config[tables_prefix]albums.title, $config[tables_prefix]albums.album_id) when $table_name.dvd_id>0 then $config[tables_prefix]dvds.title when $table_name.profile_id>0 then u2.username end as object, case when $table_name.subscription_id>0 then 1 else 0 end is_subscription";
	$table_summary_selector = "sum(tokens) as tokens_spent, sum(tokens_revenue) as tokens_revenue, '0000-00-00 00:00:00' as added_date, '0000-00-00 00:00:00' as expiry_date";
	$table_summary_field_name = "user";
	$table_projector = "$table_name left join $config[tables_prefix]users on $config[tables_prefix]users.user_id=$table_name.user_id
...
if ($_SESSION['save'][$page_name]['se_object_type_id'] > 0)
{
	switch ($_SESSION['save'][$page_name]['se_object_type_id'])
	{
		...
		case 5:
			...
			break;
		case 20:
			$where .= " and $table_name.profile_id>0";
			break;
		case 21:
			$where .= " and $table_name.messages>0";
			break;
		default:
			$where .= " and 1=0";
	}
...
if ($_SESSION['save'][$page_name]['se_object_id'] != '')
{
	switch ($_SESSION['save'][$page_name]['se_object_type_id'])
	{
		...
		case 5:
			...
			break;
		case 20:
			$where .= " and $table_name.profile_id=" . intval($_SESSION['save'][$page_name]['se_object_id']);
			break;
		case 21:
			break;
		default:
...
} elseif ($sort_by == 'is_subscription')
{
	$sort_by = "$table_name.subscription_id";
...
grid_presets_end($grid_presets, $page_name, 'stats_users_purchases');
