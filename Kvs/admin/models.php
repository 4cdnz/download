...
grid_presets_start($grid_presets, $page_name, 'models');
...
$list_gender_values = [0 => ' '];
$list_hair_values = [0 => ' '];
$list_eye_color_values = [0 => ' '];

foreach (KvsObjectTypeModel::get_instance()->get_fields() as $field)
{
	if ($field->is_choice())
	{
		$field->get_choice_options();
	}
}

$lookups = mr2array(sql_pr("select * from $config[tables_prefix]lookups where object_type_id=4 order by value"));
foreach ($lookups as $lookup)
{
	$array = null;
	$lookup['titles'] = @json_decode($lookup['titles'], true);
	if ($lookup['field_name'] == 'gender_id')
	{
		$list_gender_values[$lookup['value']] = $lookup['titles'][$lang['system']['language_code']];
	} elseif ($lookup['field_name'] == 'hair_id')
	{
		$list_hair_values[$lookup['value']] = $lookup['titles'][$lang['system']['language_code']];
	} elseif ($lookup['field_name'] == 'eye_color_id')
	{
		$list_eye_color_values[$lookup['value']] = $lookup['titles'][$lang['system']['language_code']];
	}
}
...
$table_fields[] = array('id' => 'status_id',       'title' => $lang['categorization']['model_field_status'],         'is_default' => 0, 'type' => 'choice', 'values' => $list_status_values, 'filter_ids' => ['se_status_id' => 'status_id']);
...
$table_fields[] = array('id' => 'model_group',     'title' => $lang['categorization']['model_field_group'],          'is_default' => 1, 'type' => 'refid', 'link' => 'models_groups.php?action=change&item_id=%id%', 'link_id' => 'model_group_id', 'permission' => 'models_groups|view', 'filter_ids' => ['se_model_group_id' => 'model_group_id']);
...
$table_fields[] = array('id' => 'country',         'title' => $lang['categorization']['model_field_country'],        'is_default' => 0, 'type' => 'choice', 'values' => $list_country_values);
...
$table_fields[] = array('id' => 'tags',            'title' => $lang['categorization']['model_field_tags'],           'is_default' => 0, 'type' => 'list', 'link' => 'tags.php?action=change&item_id=%id%', 'permission' => 'tags|view', 'filter_ids' => ['se_tag' => 'title']);
$table_fields[] = array('id' => 'categories',      'title' => $lang['categorization']['model_field_categories'],     'is_default' => 0, 'type' => 'list', 'link' => 'categories.php?action=change&item_id=%id%', 'permission' => 'categories|view', 'filter_ids' => ['se_category' => 'title']);
...
$table_fields[] = array('id' => "gallery_url",           'title' => $lang['categorization']['model_field_gallery_url'],           'is_default' => 0, 'type' => 'url');
...
$table_fields[] = array('id' => 'avg_videos_rating',     'title' => $lang['categorization']['model_field_avg_videos_rating'],     'is_default' => 0, 'type' => 'float',    'show_in_sidebar' => 1, 'ifdisable_zero' => 1);
$table_fields[] = array('id' => 'avg_videos_popularity', 'title' => $lang['categorization']['model_field_avg_videos_popularity'], 'is_default' => 0, 'type' => 'traffic',  'show_in_sidebar' => 1, 'ifdisable_zero' => 1);
$table_fields[] = array('id' => 'avg_albums_rating',     'title' => $lang['categorization']['model_field_avg_albums_rating'],     'is_default' => 0, 'type' => 'float',    'show_in_sidebar' => 1, 'ifdisable_zero' => 1);
$table_fields[] = array('id' => 'avg_albums_popularity', 'title' => $lang['categorization']['model_field_avg_albums_popularity'], 'is_default' => 0, 'type' => 'traffic',  'show_in_sidebar' => 1, 'ifdisable_zero' => 1);
$table_fields[] = array('id' => 'avg_posts_rating',      'title' => $lang['categorization']['model_field_avg_posts_rating'],      'is_default' => 0, 'type' => 'float',    'show_in_sidebar' => 1, 'ifdisable_zero' => 1);
$table_fields[] = array('id' => 'avg_posts_popularity',  'title' => $lang['categorization']['model_field_avg_posts_popularity'],  'is_default' => 0, 'type' => 'traffic',  'show_in_sidebar' => 1, 'ifdisable_zero' => 1);
...
$search_fields[] = array('id' => 'gallery_url', 'title' => $lang['categorization']['model_field_gallery_url']);
...
if ($_SESSION['save'][$page_name]['se_model_group'] != '')
{
	...
	{
		$where .= " and $table_name.model_group_id=$model_group_id";
...
grid_presets_end($grid_presets, $page_name, 'models');
...
if (in_array($_POST['action'], array('add_new_complete', 'change_complete')))
{
	...
	if ($_POST['model_group'] != '')
	{
		if (!in_array('models_groups|add', $_SESSION['permissions']) && mr2number(sql_pr("select count(*) from $config[tables_prefix]models_groups where title=?", $_POST['model_group'])) == 0)
		{
			$errors[] = get_aa_error('invalid_group', $lang['videos']['model_field_group']);
		}
	}
	...
	if ($_POST['country'] != '')
	{
		if (mr2number(sql_pr("select count(*) from $config[tables_prefix]list_countries where title=? and is_system=0", $_POST['country'])) == 0)
		{
			$errors[] = get_aa_error('invalid_country', $lang['categorization']['model_field_country']);
		}
	}
	...
	{
		$_POST['age'] = 0;
		if (validate_field('calendar_range', $_POST, $lang['categorization']['model_field_age'], ['range_start' => 'birth_date', 'range_end' => 'death_date']))
		{
			if ($_POST['birth_date'] != '' && $_POST['birth_date'] != '0000-00-00' && $_POST['birth_date'] != '0000-00-00 00:00:00')
			{
				$birth_date = date('Y-m-d', strtotime($_POST['birth_date']));
			}

			if ($_POST['death_date'] != '' && $_POST['death_date'] != '0000-00-00' && $_POST['death_date'] != '0000-00-00 00:00:00')
			{
			...
			if ($birth_date != '0000-00-00')
			{
				...
				{
					$_POST['age'] = get_age(strtotime($birth_date), strtotime($death_date));
				} else
				{
					$_POST['age'] = get_age(strtotime($birth_date));
				}
			}
		}
	...
	if (!is_array($errors))
	{
		...
		if ($_POST['country'] != '')
		{
			$_POST['country'] = mr2string(sql_pr("select max(country_code) from $config[tables_prefix]list_countries where title=?", $_POST['country']));
		}

		$_POST['model_group_id'] = 0;
		if ($_POST['model_group'] != '')
		{
			$_POST['model_group_id'] = mr2number(sql_pr("select model_group_id from $config[tables_prefix]models_groups where title=?", $_POST['model_group']));
			if ($_POST['model_group_id'] == 0)
			{
				$model_group_dir = get_correct_dir_name($_POST['model_group']);
				$temp_dir = $model_group_dir;
				for ($it = 2; $it < 999999; $it++)
				{
					if (mr2number(sql_pr("select count(*) from $config[tables_prefix]models_groups where dir=?", $temp_dir)) == 0)
					{
						$model_group_dir = $temp_dir;
						break;
					}
					$temp_dir = $model_group_dir . $it;
				}
				$_POST['model_group_id'] = sql_insert("insert into $config[tables_prefix]models_groups set title=?, dir=?, added_date=?", $_POST['model_group'], $model_group_dir, date('Y-m-d H:i:s'));
				sql_pr("insert into $config[tables_prefix]admin_audit_log set user_id=?, username=?, action_id=100, object_id=?, object_type_id=14, added_date=?", $_SESSION['userdata']['user_id'], $_SESSION['userdata']['login'], $_POST['model_group_id'], date('Y-m-d H:i:s'));
			}
		}

		if ($_POST['action'] == 'add_new_complete')
		{
			$item_id = sql_insert("insert into $table_name set model_group_id=?, title=?, dir=?, alias=?, description=?, status_id=?, screenshot1=?, screenshot2=?, rating=?, rating_amount=?, access_level_id=?, country=?, state=?, city=?, height=?, weight=?, hair_id=?, eye_color_id=?, measurements=?, gender_id=?, birth_date=?, death_date=?, age=?,
		...
		} else
		{
			...
			sql_pr("update $table_name set model_group_id=?, title=?, dir=?, alias=?, description=?, status_id=?, rating=?, rating_amount=?, access_level_id=?, country=?, state=?, city=?, height=?, weight=?, hair_id=?, eye_color_id=?, measurements=?, gender_id=?, birth_date=?, death_date=?, age=?, custom1=?, custom2=?, custom3=?, custom4=?, custom5=?, custom6=?, custom7=?, custom8=?, custom9=?, custom10=? where $table_key_name=?",
...
if ($_REQUEST['batch_action'] != '')
{
	...
	} elseif (is_array($_REQUEST['row_select']) && array_cnt($_REQUEST['row_select']) > 0)
	{
		...
		if ($_REQUEST['batch_action'] == 'delete')
		{
			...
			foreach ($data as $k => $v)
			{
				rmdir_recursive("$config[content_path_models]/$v[model_id]");
			...
			sql("delete from $config[tables_prefix]users_subscriptions where subscribed_object_id in ($row_select) and subscribed_object_type_id=4");
...
if ($_GET['action'] == 'change')
{
	...
	if ($_POST['country'] !== '')
	{
		$_POST['country'] = mr2string(sql_pr("select title from $config[tables_prefix]list_countries where country_code=? and language_code=?", $_POST['country'], $lang['system']['language_code']));
	}
...
$smarty->assign('list_gender_values', $list_gender_values);
$smarty->assign('list_hair_values', $list_hair_values);
$smarty->assign('list_eye_color_values', $list_eye_color_values);
