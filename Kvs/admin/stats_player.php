...
grid_presets_start($grid_presets, $page_name, 'stats_player');
...
if ($list_grouping == "date" || $list_grouping == "referer" || $list_grouping == "country" || $list_grouping == "device" || $list_grouping == "embed_profile")
{
	...
	{
		$table_fields[] = array('id' => 'player_adblock',   'title' => $lang['stats']['player_field_event_player_adblock'],     'is_default' => 1, 'type' => 'double',  'ifdisable_zero' => 1, 'format' => 'percent');
	}
	...
	$table_fields[] = array('id' => 'video_rate_changes',   'title' => $lang['stats']['player_field_event_video_rate_changed'], 'is_default' => 1, 'type' => 'traffic', 'ifdisable_zero' => 1);
	...
	$table_fields[] = array('id' => 'start_ad_ctr',         'title' => $lang['stats']['player_field_event_start_ad_ctr'],       'is_default' => 0, 'type' => 'double',  'ifdisable_zero' => 1, 'format' => 'percent');
	...
	$table_fields[] = array('id' => 'pre_ad_ctr',           'title' => $lang['stats']['player_field_event_pre_ad_ctr'],         'is_default' => 0, 'type' => 'double',  'ifdisable_zero' => 1, 'format' => 'percent');
	...
	$table_fields[] = array('id' => 'post_ad_ctr',          'title' => $lang['stats']['player_field_event_post_ad_ctr'],        'is_default' => 0, 'type' => 'double',  'ifdisable_zero' => 1, 'format' => 'percent');
	...
	$table_fields[] = array('id' => 'pause_ad_ctr',         'title' => $lang['stats']['player_field_event_pause_ad_ctr'],       'is_default' => 0, 'type' => 'double',  'ifdisable_zero' => 1, 'format' => 'percent');
	...
	$table_fields[] = array('id' => 'popunder_ad_clicks',   'title' => $lang['stats']['player_field_event_popunder_ad_clicks'], 'is_default' => 0, 'type' => 'traffic', 'ifdisable_zero' => 1);
}
...
$temp_stats_columns = ['player_loads', 'player_fullscreens', 'player_mutes', 'player_unmutes', 'video_starts', 'video_pauses', 'video_skips', 'video_rate_changes', 'video_ends', 'video_errors', 'start_ad_views', 'start_ad_clicks', 'start_ad_errors', 'pre_ad_views', 'pre_ad_clicks', 'pre_ad_skips', 'pre_ad_errors', 'post_ad_views', 'post_ad_clicks', 'post_ad_skips', 'post_ad_errors', 'pause_ad_views', 'pause_ad_clicks', 'pause_ad_errors', 'popunder_ad_clicks'];
...
if ($list_grouping == "date")
{
	$table_selector = "$table_name.added_date, case when sum($table_name.player_total) > 0 then 100 * sum(greatest(cast($table_name.player_total as signed)-cast($table_name.player_loads as signed), 0)) / sum($table_name.player_total) else 0 end as player_adblock, $temp_stats_columns_sum_str";
	$table_summary_selector = "case when sum($table_name.player_total) > 0 then 100 * sum(greatest(cast($table_name.player_total as signed)-cast($table_name.player_loads as signed), 0)) / sum($table_name.player_total) else 0 end as player_adblock, $temp_stats_columns_sum_str";
	$table_summary_field_name = "added_date";
	$table_projector = $table_name;
	$table_group_by = "$table_name.added_date";
} elseif ($list_grouping == "referer")
{
	$table_selector = "max($config[tables_prefix_multi]stats_referers_list.title) as referer, case when sum($table_name.player_total) > 0 then 100 * sum(greatest(cast($table_name.player_total as signed)-cast($table_name.player_loads as signed), 0)) / sum($table_name.player_total) else 0 end as player_adblock, $temp_stats_columns_sum_str";
	$table_summary_selector = "case when sum($table_name.player_total) > 0 then 100 * sum(greatest(cast($table_name.player_total as signed)-cast($table_name.player_loads as signed), 0)) / sum($table_name.player_total) else 0 end as player_adblock, $temp_stats_columns_sum_str";
...
{
	$table_selector = "max($config[tables_prefix]list_countries.title) as country, case when sum($table_name.player_total) > 0 then 100 * sum(greatest(cast($table_name.player_total as signed)-cast($table_name.player_loads as signed), 0)) / sum($table_name.player_total) else 0 end as player_adblock, $temp_stats_columns_sum_str";
	$table_summary_selector = "case when sum($table_name.player_total) > 0 then 100 * sum(greatest(cast($table_name.player_total as signed)-cast($table_name.player_loads as signed), 0)) / sum($table_name.player_total) else 0 end as player_adblock, $temp_stats_columns_sum_str";
...
{
	$table_selector = "$table_name.device, case when sum($table_name.player_total) > 0 then 100 * sum(greatest(cast($table_name.player_total as signed)-cast($table_name.player_loads as signed), 0)) / sum($table_name.player_total) else 0 end as player_adblock, $temp_stats_columns_sum_str";
	$table_summary_selector = "case when sum($table_name.player_total) > 0 then 100 * sum(greatest(cast($table_name.player_total as signed)-cast($table_name.player_loads as signed), 0)) / sum($table_name.player_total) else 0 end as player_adblock, $temp_stats_columns_sum_str";
...
grid_presets_end($grid_presets, $page_name, 'stats_player');
