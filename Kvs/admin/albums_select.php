...
if ($_POST['action'] == 'select_complete')
{
	...
	validate_field('empty', $_POST['selector'], $lang['albums']['select_field_list_items']);
	...
	foreach ($selector as $album_url)
	{
		...
		if (is_url($album_url))
		{
			...
			$pattern_check = $website_ui_data['WEBSITE_LINK_PATTERN_ALBUM'];
			if ($pattern_check[0] != '/')
			{
			...
			unset($temp);
			if (preg_match("|$pattern_regexp|is", parse_url($album_url, PHP_URL_PATH), $temp))
			{
			...
			} else
			{
				...
				{
					if (strpos($album_url, str_replace('www.', '', parse_url($satellite['project_url'], PHP_URL_HOST))) !== false && $satellite['website_ui_data'])
					{
						$satellite['website_ui_data'] = @unserialize($satellite['website_ui_data']) ?: [];
						$pattern_check = $satellite['website_ui_data']['WEBSITE_LINK_PATTERN_ALBUM'];
						if ($pattern_check[0] != '/')
						{
							$pattern_check = "/$pattern_check";
						}
		...
		} elseif (is_numeric($album_url) && intval($album_url) > 0)
		{
			$album_ids[] = intval($album_url);
		} elseif ($album_url)
		{
			$errors[] = get_aa_error('invalid_album_page_url', $album_url);
		}
	...
	if ($where != '' && array_cnt($album_ids) > 0)
	{
		$album_ids_str = implode(', ', $album_ids);
		$album_ids = mr2array_list(sql("select album_id from $config[tables_prefix]albums where album_id in ($album_ids_str) $where"));
	}

	if (array_cnt($album_ids) == 0)
	{
		validate_field('empty', '', $lang['albums']['select_field_list_items']);
	}
	...
	if (!is_array($errors))
	{
		switch ($_POST['operation'])
		{
			case 'list':
				...
				file_put_contents("$config[temporary_path]/mass-select-$rnd.dat", serialize(['ids' => $album_ids]));
				return_ajax_success("albums.php?no_filter=true&se_file_ids=$rnd");
				break;
			case 'mass_edit':
				$rnd = mt_rand(10000000, 99999999);
				file_put_contents("$config[temporary_path]/mass-edit-$rnd.dat", serialize(['all' => 0, 'ids' => $album_ids]));
				return_ajax_success("albums_mass_edit.php?edit_id=$rnd", 1);
				break;
			case 'export':
				$rnd = mt_rand(10000000, 99999999);
				file_put_contents("$config[temporary_path]/mass-export-$rnd.dat", serialize(['ids' => $album_ids]));
				return_ajax_success("albums_export.php?se_file_ids=$rnd");
				break;
			case 'mark_deleted':
				$rnd = mt_rand(10000000, 99999999);
				file_put_contents("$config[temporary_path]/delete-albums-$rnd.dat", serialize(['ids' => $album_ids]));
