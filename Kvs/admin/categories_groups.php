...
grid_presets_start($grid_presets, $page_name, 'category_groups');
...
$table_fields[] = array('id' => 'status_id',         'title' => $lang['categorization']['category_group_field_status'],      'is_default' => 0, 'type' => 'choice', 'values' => $list_status_values, 'filter_ids' => ['se_status_id' => 'status_id']);
...
$table_fields[] = array('id' => 'categories_amount', 'title' => $lang['categorization']['category_group_field_categories'],  'is_default' => 1, 'show_in_sidebar' => 1, 'type' => 'number', 'link' => 'categories.php?no_filter=true&se_category_group=%id%', 'link_id' => 'title', 'permission' => 'categories|view', 'ifdisable_zero' => 1);
...
grid_presets_end($grid_presets, $page_name, 'category_groups');
...
if ($_REQUEST['batch_action'] != '')
{
	...
	} elseif (is_array($_REQUEST['row_select']) && array_cnt($_REQUEST['row_select']) > 0)
	{
		...
		if ($_REQUEST['batch_action'] == 'delete')
		{
			...
			foreach ($data as $k => $v)
			{
				rmdir_recursive("$config[content_path_categories]/groups/$v[$table_key_name]");
