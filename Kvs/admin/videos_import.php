...
if ($_GET['action']!='back_import' && !isset($_GET['preset_id']) && array_cnt($_POST)==0 && is_array($VIDEOS_IMPORT_PRESETS))
{
	foreach ($VIDEOS_IMPORT_PRESETS as $k=>$preset)
	{
...
if ($_POST['action']=='start_import')
{
	...
	foreach ($_POST['fields'] as $field)
	{
		...
		if ($field)
		{
			if ($field != 'skip')
			{
				...
				{
					$errors[] = get_aa_error('import_fields_duplication', str_replace('%1%', $i, $lang['videos']['import_export_field']));
				} elseif (!$is_new_import && (in_array($field, ['video_file', 'gallery_url', 'server_group', 'screenshot_main_number', 'screenshot_main_source', 'overview_screenshots_zip', 'overview_screenshots_sources', 'poster_main_number', 'posters_zip', 'posters_sources', 'max_duration', 'offset_from_start', 'offset_from_end']) || strpos($field, 'format_video_') === 0))
				{
					$errors[] = get_aa_error('import_fields_update_not_supported', str_replace('%1%', $i, $lang['videos']['import_export_field']));
				} elseif (!$is_new_import && $field != 'video_id')
				{
					$permission_id = "videos|edit_{$field}";
					switch ($field)
					{
						case 'directory':
							$permission_id = 'videos|edit_dir';
							break;
						case 'video_url':
							$permission_id = 'videos|edit_url';
							break;
						case 'embed_code':
							$permission_id = 'videos|edit_embed';
							break;
						case 'relative_post_date':
							$permission_id = 'videos|edit_post_date';
							break;
						case 'rating':
						case 'rating_percent':
						case 'rating_amount':
						case 'video_viewed':
							$permission_id = 'videos|edit_all';
							break;
						case 'custom1':
						case 'custom2':
						case 'custom3':
							$permission_id = 'videos|edit_custom';
							break;
					}
					...
					foreach ($category_groups as $category_group)
					{
						...
						{
							$permission_id = 'videos|edit_categories';
							break;
						}
					}
					if (!in_array($permission_id, $_SESSION['permissions']))
					{
						$errors[] = get_aa_error('import_fields_update_forbidden', str_replace('%1%', $i, $lang['videos']['import_export_field']));
					}
				}
	...
	if ($is_error)
	{
		$errors[] = get_aa_error('import_fields_required', $lang['videos']['import_divider_fields']);
	} elseif (!$is_new_import && $is_id_error)
	{
		$errors[] = get_aa_error('import_fields_update_id_required', $lang['videos']['import_divider_fields']);
	}
	...
	if ($is_new_import)
	{
		if ($_POST['content_source'] != '')
		{
			...
			{
				$errors[] = get_aa_error('invalid_content_source', $lang['videos']['import_field_content_source']);
			}
		}
		...
		if (array_cnt($_POST['user_ids'])>0)
		{
			foreach ($_POST['user_ids'] as $user_id)
			{
				...
				{
					$errors[] = get_aa_error('invalid_user', $lang['videos']['import_field_users']);
					break;
				}
			}
		}
		...
		{
			validate_field('time_range', $_POST, $lang['videos']['import_field_post_date'], ['is_required' => 1, 'range_start' => 'post_time_randomization_from', 'range_end' => 'post_time_randomization_to']);
		}
		...
		if ($is_post_date_randomization == 1)
		{
			...
			{
				validate_field('calendar_range', $_POST, $lang['videos']['import_field_post_date'], ['is_fully_required' => 1, 'same_allowed' => 1, 'range_start' => 'post_date_randomization_from', 'range_end' => 'post_date_randomization_to']);
			} else
			{
				validate_field('int_range', $_POST, $lang['videos']['import_field_post_date'], ['is_fully_required' => 1, 'same_allowed' => 1, 'range_start' => 'relative_post_date_randomization_from', 'range_end' => 'relative_post_date_randomization_to']);
			}
	...
	if (!is_array($errors))
	{
		...
		exec("$config[php_path] $config[project_path]/admin/background_import.php $rnd validation $lang $admin_id > /dev/null 2>&1 &");
...
if (isset($_POST['save_default']) && intval($_POST['import_id']) > 0)
{
	...
	sql_pr("insert into $config[tables_prefix]background_imports set import_id=?, task_id=?, admin_id=?, status_id=0, type_id=?, threads=?, options=?, added_date=?",
			$import_id, $task_id, $admin_id, $import_task['import_mode'] == 1 ? 3 : 1, intval($import_task['threads']), serialize($import_task), date("Y-m-d H:i:s")
	);
	...
	{
		$_SESSION['messages'][] = $lang['videos']['success_message_import_started'];
...
if ($_POST['users'] == '')
{
	...
	{
		$_POST['users'] = mr2array(sql_pr("select user_id, username from $config[tables_prefix]users where username=?", $options['DEFAULT_USER_IN_ADMIN_ADD_VIDEO']));
	}
} else
{
	// older versions
