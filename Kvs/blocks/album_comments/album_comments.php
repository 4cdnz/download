...
function album_commentsShow($block_config,$object_id)
{
	...
	} elseif ($album_info['is_private']==1)
	{
		if (intval($memberzone_data['ENABLE_TOKENS_PRIVATE_ALBUM'])==1)
		{
			if (intval($album_info['tokens_required'])==0)
			{
				$album_info['tokens_required']=intval($memberzone_data['DEFAULT_TOKENS_PRIVATE_ALBUM']);
			}
		} else {
			$album_info['tokens_required']=0;
		}
		...
		if (intval($memberzone_data['ENABLE_TOKENS_PUBLIC_ALBUM'])==1)
		{
			if (intval($album_info['tokens_required'])==0)
			{
				$album_info['tokens_required']=intval($memberzone_data['DEFAULT_TOKENS_PUBLIC_ALBUM']);
			}
		} else {
			$album_info['tokens_required']=0;
		}
...
function album_commentsAsync($block_config)
{
	...
	if ($_REQUEST['action']=='add_comment' && (intval($_REQUEST['image_id'])>0 || intval($_REQUEST['album_id'])>0))
	{
		...
		if (!is_array($errors))
		{
			if ($user_id>0)
			{
				...
				if ($approved==0)
				{
					...
					if ($image_id > 0)
					{
						sql("update $config[tables_prefix]albums_images set comments_count=(select count(*) from $config[tables_prefix]comments where object_sub_id=$config[tables_prefix]albums_images.image_id and object_type_id=2 and is_approved=1) where image_id=$image_id");
					}
